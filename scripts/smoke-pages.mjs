...
const cases = [
  route("/", ["Meet KAI", "Start with KAI", "Parents", "Privacy"], { actionables: ["a[href='/crisis']"] }),
  ...
  route("/home", ["KAI home", "Good to see you.", "next move"], { actionables: ["textarea", "button"], kaiChatHandoff: true }),
...
async function assertQuickActionLinks(client) {
  ...
  await waitForLoad(client);
  await waitForRoot(client);
  await waitForExpectedSnapshot(client, { expectedText: ["KAI home", "Good to see you.", "next move"], actionables: ["textarea", "button"] });
  await clickSelector(client, "button[aria-label='Open KAI tasks']");

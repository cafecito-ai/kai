...
import {
  ...
  type DecisionArticle,
  type DecisionCategory
} from "../../lib/decision-primer";
...
export function DecisionPrimer({ onRead }: Props) {
  const [openId, setOpenId] = useState<string | null>(null);
  const [readSet, setReadSet] = useState<Set<string>>(new Set());

  const grouped = useMemo(() => {
    const byCat: Record<DecisionCategory, DecisionArticle[]> = {
      decision_frames: [],
      biases_to_know: [],
      reversibility: [],
      when_to_defer: [],
    ...
    for (const article of DECISION_ARTICLES) byCat[article.category].push(article);
    return byCat;
  }, []);

  function toggle(article: DecisionArticle) {
  ...
  return (
    <section className="rounded-kai border border-line bg-white p-5 shadow-sm">
      ...
      <p className="eyebrow">decision-making</p>
      <h2 className="mt-2 font-display text-3xl font-black tracking-normal">How to decide, not what to decide.</h2>
      <p className="mt-2 text-sm leading-6 text-muted">
        Reversibility, cognitive biases, when to defer, when to commit, who to ask, and how to recover when you decided wrong. The frame is yours; the choices are yours.
      </p>

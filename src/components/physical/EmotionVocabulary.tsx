...
import {
  EMOTION_ARTICLES,
  EMOTION_CATEGORY_LABEL,
  type EmotionArticle,
  type EmotionCategory
} from "../../lib/emotion-vocabulary";

type Props = {
  onRead?: (input: { articleId: string }) => void;
};

const CATEGORY_ORDER: EmotionCategory[] = [
  "body_cues",
  "anger_family",
  "sadness_family",
  "fear_family",
...
export function EmotionVocabulary({ onRead }: Props) {
  const [openId, setOpenId] = useState<string | null>(null);
  const [readSet, setReadSet] = useState<Set<string>>(new Set());

  const grouped = useMemo(() => {
    const byCat: Record<EmotionCategory, EmotionArticle[]> = {
      anger_family: [],
      sadness_family: [],
      fear_family: [],
      joy_family: [],
    ...
    for (const article of EMOTION_ARTICLES) byCat[article.category].push(article);
    return byCat;
  }, []);

  function toggle(article: EmotionArticle) {
  ...
  return (
    <section className="rounded-kai border border-line bg-white p-5 shadow-sm">
      ...
      <p className="eyebrow">emotion vocabulary</p>
      <h2 className="mt-2 font-display text-3xl font-black tracking-normal">Name it specifically.</h2>
      <p className="mt-2 text-sm leading-6 text-muted">
        Naming an emotion precisely turns its volume down — one of the better-evidenced findings in emotion science. Five families, body cues, the difference between guilt and shame.
      </p>

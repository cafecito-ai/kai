...
import {
  ...
  type FocusArticle,
  type FocusCategory
} from "../../lib/focus-study-primer";

type Props = {
  onRead?: (input: { articleId: string }) => void;
};

const CATEGORY_ORDER: FocusCategory[] = [
...
export function FocusStudyPrimer({ onRead }: Props) {
  const [openId, setOpenId] = useState<string | null>(null);
  const [readSet, setReadSet] = useState<Set<string>>(new Set());
  const addEvent = useProgressStore((state) => state.addEvent);

  const grouped = useMemo(() => {
    const byCat: Record<FocusCategory, FocusArticle[]> = {
      myth_of_multitasking: [],
      time_blocks: [],
      environment: [],
      retrieval_practice: [],
    ...
    for (const article of FOCUS_ARTICLES) byCat[article.category].push(article);
    return byCat;
  }, []);

  function toggle(article: FocusArticle) {
  ...
  return (
    <section className="rounded-kai border border-line bg-white p-5 shadow-sm">
      ...
      <p className="eyebrow">focus + study</p>
      <h2 className="mt-2 font-display text-3xl font-black tracking-normal">Studying smarter, not longer.</h2>
      <p className="mt-2 text-sm leading-6 text-muted">
        Most teens spend study time on the wrong techniques. The cognitive science is unusually clear about what actually builds learning.
      </p>

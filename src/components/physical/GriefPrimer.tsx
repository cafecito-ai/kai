...
import {
  GRIEF_ARTICLES,
  GRIEF_CATEGORY_LABEL,
  type GriefArticle,
  type GriefCategory
} from "../../lib/grief-primer";

type Props = {
  onRead?: (input: { articleId: string }) => void;
};

const CATEGORY_ORDER: GriefCategory[] = [
  "what_grief_is",
  "death_of_loved_one",
  "non_death_loss",
  "anticipatory_grief",
...
export function GriefPrimer({ onRead }: Props) {
  const [openId, setOpenId] = useState<string | null>(null);
  const [readSet, setReadSet] = useState<Set<string>>(new Set());

  const grouped = useMemo(() => {
    const byCat: Record<GriefCategory, GriefArticle[]> = {
      what_grief_is: [],
      death_of_loved_one: [],
      non_death_loss: [],
      anticipatory_grief: [],
    ...
    for (const article of GRIEF_ARTICLES) byCat[article.category].push(article);
    return byCat;
  }, []);

  function toggle(article: GriefArticle) {
  ...
  return (
    <section className="rounded-kai border border-line bg-white p-5 shadow-sm">
      ...
      <p className="eyebrow">grief + loss</p>
      <h2 className="mt-2 font-display text-3xl font-black tracking-normal">Grief, plainly.</h2>
      <p className="mt-2 text-sm leading-6 text-muted">
        Death, non-death loss, anticipatory grief, when grief gets stuck. Most grief content for teens is wrong. This is what's true, based on the research and the lived experience.
      </p>

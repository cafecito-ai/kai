...
import {
  IDENTITY_ARTICLES,
  IDENTITY_CATEGORY_LABEL,
  type IdentityArticle,
  type IdentityCategory
} from "../../lib/identity-primer";

type Props = {
  onRead?: (input: { articleId: string }) => void;
};

const CATEGORY_ORDER: IdentityCategory[] = [
  "values",
  "comparison",
  "authenticity",
  "change",
...
export function IdentityPrimer({ onRead }: Props) {
  const [openId, setOpenId] = useState<string | null>(null);
  const [readSet, setReadSet] = useState<Set<string>>(new Set());

  const grouped = useMemo(() => {
    const byCat: Record<IdentityCategory, IdentityArticle[]> = {
      values: [],
      comparison: [],
      authenticity: [],
      change: [],
    ...
    for (const article of IDENTITY_ARTICLES) byCat[article.category].push(article);
    return byCat;
  }, []);

  function toggle(article: IdentityArticle) {
  ...
  return (
    <section className="rounded-kai border border-line bg-white p-5 shadow-sm">
      ...
      <p className="eyebrow">identity + self-knowledge</p>
      <h2 className="mt-2 font-display text-3xl font-black tracking-normal">Knowing yourself, slowly.</h2>
      <p className="mt-2 text-sm leading-6 text-muted">
        Values, comparison, the right to change, the voice in your head. Curiosity over conclusion. The product asks; it doesn't tell you who to be.
      </p>

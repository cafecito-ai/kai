...
} from "../../lib/purpose-primer";

type Props = {
  onRead?: (input: { articleId: string }) => void;
};

const CATEGORY_ORDER: PurposeCategory[] = [
  "what_purpose_is",
  "passion_vs_practice",
...
export function PurposePrimer({ onRead }: Props) {
  const [openId, setOpenId] = useState<string | null>(null);
  const [readSet, setReadSet] = useState<Set<string>>(new Set());

  const grouped = useMemo(() => {
    const byCat: Record<PurposeCategory, PurposeArticle[]> = {
      what_purpose_is: [],
      passion_vs_practice: [],
      existential_questions: [],
      contribution_and_service: [],
    ...
    for (const article of PURPOSE_ARTICLES) byCat[article.category].push(article);
    return byCat;
  }, []);

  function toggle(article: PurposeArticle) {
  ...
  return (
    <section className="rounded-kai border border-line bg-white p-5 shadow-sm">
      ...
      <p className="eyebrow">purpose + meaning</p>
      <h2 className="mt-2 font-display text-3xl font-black tracking-normal">Built, not found.</h2>
      <p className="mt-2 text-sm leading-6 text-muted">
        The honest version of purpose research. Existential questions, passion-vs-practice, career-without-pressure, contribution, and what to do when meaning falters.
      </p>

...
const CATEGORY_ORDER: RelCategory[] = [
  "friendship",
  "conflict",
  "romantic",
  "loneliness",
...
export function RelationshipsPrimer({ onRead }: Props) {
  const [openId, setOpenId] = useState<string | null>(null);
  const [readSet, setReadSet] = useState<Set<string>>(new Set());

  const grouped = useMemo(() => {
    const byCat: Record<RelCategory, RelArticle[]> = {
      friendship: [],
      conflict: [],
      romantic: [],
      loneliness: [],
    ...
    for (const article of REL_ARTICLES) byCat[article.category].push(article);
    return byCat;
  }, []);

  function toggle(article: RelArticle) {
  ...
  return (
    <section className="rounded-kai border border-line bg-white p-5 shadow-sm">
      ...
      <p className="eyebrow">friendships + relationships</p>
      <h2 className="mt-2 font-display text-3xl font-black tracking-normal">People stuff.</h2>
      <p className="mt-2 text-sm leading-6 text-muted">
        Friendships, conflict + repair, romantic basics, loneliness, warning signs, and the boundary muscle. The product describes patterns, doesn't label people.
      </p>

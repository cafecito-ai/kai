...
const CATEGORY_ORDER: StressCategory[] = [
  "academic",
  "perfectionism",
  "performance",
  "social",
...
/**
 * Stress + pressure primer. Same collapsible-article shape as
 * BodyLiteracy (#33), NutritionPrimer (#35), DigitalWellbeing (#39).
 *
 * Topic is sensitive; copy is reviewed for: no diagnosis, no "just relax",
 * no dismissal, direct-to-clinician for chronic patterns.
 */
export function StressPrimer({ onRead }: Props) {
  const [openId, setOpenId] = useState<string | null>(null);
  const [readSet, setReadSet] = useState<Set<string>>(new Set());

  const grouped = useMemo(() => {
    const byCat: Record<StressCategory, StressArticle[]> = {
      academic: [],
      perfectionism: [],
      performance: [],
      social: [],
    ...
    for (const article of STRESS_ARTICLES) byCat[article.category].push(article);
    return byCat;
  }, []);

  function toggle(article: StressArticle) {
  ...
  return (
    <section className="rounded-kai border border-line bg-white p-5 shadow-sm">
      ...
      <p className="eyebrow">stress + pressure</p>
      <h2 className="mt-2 font-display text-3xl font-black tracking-normal">Stress is information.</h2>
      <p className="mt-2 text-sm leading-6 text-muted">
        Stress with recovery builds you. Stress without recovery wears you. Knowing which one you're in changes what to do.
      </p>

...
export function NextLoopCard({ context = "default" }: { context?: "default" | "compact" }) {
  ...
  return (
    <section className={`w-full max-w-full min-w-0 overflow-hidden rounded-[28px] border border-[#0A0A0A0F] bg-white p-5 shadow-[0_1px_2px_rgba(10,10,10,0.04),0_12px_36px_rgba(10,10,10,0.08)] ${compact ? "" : "sm:p-6"}`}>
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <h2 className="mt-2 break-words font-display text-2xl font-semibold leading-tight tracking-normal text-[#111116]">{recommendation.title}</h2>
          </div>
        </div>
        {!compact && (
      ...
      <p className="mt-3 break-words text-sm font-medium leading-6 text-[#5E5E64]">{recommendation.copy}</p>
